        ...
        for row in reader:
            ...
            if name.lower() in player_name.lower():
                return {
                    ...
                    'barrel': float(row[17]) if row[17] else 0,
                }
        return None
    except:
        return None
...
if analysis_type == "Batter":
    ...
    player_name = st.text_input("Enter player name:", placeholder="e.g., Bogaerts, Xander")
        ...
        if player_name:
            with st.spinner("Fetching data from Baseball Savant..."):
                ...
                if data:
                    ...
                    st.error(f"Could not find '{player_name}' in Baseball Savant data")
        else:
            st.warning("Please enter a player name")
    
    st.markdown("---")
    st.markdown("### 📋 How to use:")
    st.markdown("1. Enter player name (Last, First format works best)")
    st.markdown("2. Click 'Analyze'")
    st.markdown("3. Check Luck Indicators for buy/sell signals")
    st.markdown("4. Check Skill Metrics for power evaluation")
    ...
    st.markdown("- **ERA > xERA by 1.00+** (unlucky but check skills)")
    st.markdown("- **Barrel% > 10%** (getting hit hard)")
    st.markdown("- **K-BB% < 10%** (poor command)")

st.markdown("---")
st.caption("Data from Baseball Savant | Built with Streamlit")
